...
class page extends Collection
{
    ...
    */
     ...
     */
    public function assignPermissionSet($permissionsArray)
    {
        $db = Loader::db();
     ...
     */
    public function getAllowedSubCollections()
    {
    ...
    */
    public function getCollectionDatePublic($dateFormat = null, $type='system')
    {
        if (!$dateFormat) {
     ...
     * @return int
     */
    public function getPendingActionUserID()
    {
     ...
     * @return string date formated like: 2009-01-01 00:00:00
    */
    public function getPendingActionDateTime($type = 'system')
    {
        if (ENABLE_USER_TIMEZONES && $type == 'user') {
            $dh = Loader::helper('date');
     ...
     * @return int
     */
    public function getPendingActionTargetCollectionID()
    {
    ...
    public function updatePermissions($args = null)
    {
        $db = Loader::db();
        if (!is_array($args)) {
        ...
        if ($args['cInheritPermissionsFrom'] == 'OVERRIDE') {
            ...
            } elseif ($args['cInheritPermissionsFrom'] == 'TEMPLATE') {
    ...
    public function move($nc, $retainOldPagePath = false)
    {
        $db = Loader::db();
    ...
    **/
    ...
    public function markPendingAction($action, $targetC = null)
    {
    ...
    public function approvePendingAction()
    {
        $db = Loader::db();
    ...
    public function rescanCollectionPath($retainOldPagePath = false)
    {
        if ($this->cParentID > 0) {
            $db = Loader::db();
    ...
    public function rescanCollectionPathIndividual($cID, $cPath, $retainOldPagePath = false)
    {
        $db = Loader::db();
    ...
    public function rescanCollectionPathChildren($cID, $cPath)
    {
        $db = Loader::db();
        $q = "select cID from Pages where cParentID = $cID";
        $r = $db->query($q);
    ...
    *
    **/
    public function updateGroups($args = null)
    {
    ...
    **/
    ...
    public function getCollectionFullPageCachingLifetimeValue()
    {
        if ($this->cCacheFullPageContentOverrideLifetime == 'default') {
            ...
            } elseif (FULL_PAGE_CACHE_LIFETIME == 'forever') {
                $lifetime = 31536000; // 1 year
            } else {
                $lifetime = CACHE_LIFETIME;
            }
