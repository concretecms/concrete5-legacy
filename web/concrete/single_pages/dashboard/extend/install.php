...
<?php } elseif ($this->controller->getTask() == 'uninstall' && $tp->canUninstallPackages()) { ?>
<?php
...
<?php
    ...
    public function sortAvailableArray($obj1, $obj2)
    {
    ...
    <?php
        ...
        <?php } elseif (is_object($mi) && $mi->hasConnectionError()) { ?>

            <?=Loader::element('dashboard/marketplace_connect_failed');?>

        <?php } elseif ($tp->canInstallPackages() && ENABLE_MARKETPLACE_SUPPORT == true) { ?>
