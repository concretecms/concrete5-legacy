...
<?php } else {	
			...
			<td valign="top">
				...
				if (count($screenshots) > 0) { 
					foreach($screenshots as $si) { ?>
						<img src="<?=$si->src?>" width="<?=$si->width?>" height="<?=$si->height?>" <?php if ($i != 0) { ?>style="display: none" <?php } ?> />	
					<?php 
					$i++;
					}
	...
	?>
		...
		<div class="dialog-buttons">
			<input type="button" class="btn primary <?php if (!$mri->getMarketplaceItemVersionForThisSite()) { ?> disabled<?php } ?> ccm-button-right" value="<?=$buttonText?>" onclick="<?=$buttonAction?>" />
