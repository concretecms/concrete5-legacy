<?php
    ...
    if (isset($_GET['cvID1']) && isset($_GET['cvID2']) && (isset($_GET['vtask']))) {
        ...
        } elseif ($_GET['vtask'] == 'compare_iframe') { ?>
...
<div class="ccm-form-area">
    ...
    <tr id="ccm-version-row<?=$v->getVersionID()?>" class="<?=$class?>">
        <td><input type="checkbox" <?php if ($vIsPending) { ?> class="cb-version-pending"<?php } elseif ($v->isApproved()) { ?> class="cb-version-active"<?php } else { ?> class="cb-version-old" <?php } ?> id="cb<?=$v->getVersionID()?>" name="vID[]" value="<?=$v->getVersionID()?>" /></td>
