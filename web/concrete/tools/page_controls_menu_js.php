...
if (isset($cp)) {
    ...
    if ($c->getCollectionPointerID() > 0) {
        ...
        if ($pendingAction == 'MOVE') {
            ...
            } elseif ($children == 0 && $cp->canApproveCollection() && (!$c->isCheckedOut() || ($c->isCheckedOut() && $c->isEditMode()))) {
                $statusMessage .= " <a href='" . DIR_REL . "/" . DISPATCHER_FILENAME . "?cID=" . $c->getCollectionID() . "&ctask=approve_pending_action" . $token . "'>" . t('Approve Delete') . "</a> | <a href='" . DIR_REL . "/" . DISPATCHER_FILENAME . "?cID=" . $c->getCollectionID() . "&ctask=clear_pending_action" . $token . "'>" . t('Cancel') . "</a>";
            }
        }
...
menuHTML += '<div class="ccm-edit-overlay-actions">';
<?php if ($cp->canWrite()) { ?>
    menuHTML += '<a id="ccm-nav-check-out" href="<?php if (!$cantCheckOut) { ?><?=DIR_REL?>/<?=DISPATCHER_FILENAME?>?cID=<?=$c->getCollectionID()?>&ctask=check-out<?=$token?><?php } else { ?>javascript:void(0);<?php } ?>" class="btn primary <?php if ($cantCheckOut) { ?> disabled <?php } ?> tooltip" <?php if ($cantCheckOut) { ?>title="<?=t('Someone has already checked this page out for editing.')?>"<?php } ?>><?=t('Edit this Page')?></a>';
<?php } ?>
...
menuHTML += '<ul>';
<?php if ($cp->canWrite()) { ?>
    menuHTML += '<li><a class="ccm-menu-icon ccm-icon-properties" id="ccm-toolbar-nav-properties" dialog-width="640" dialog-height="<?php if ($cp->canApproveCollection() && (!$c->isEditMode())) { ?>450<?php } else { ?>390<?php } ?>" dialog-append-buttons="true" dialog-modal="false" dialog-title="<?=t('Page Properties')?>" href="<?=REL_DIR_FILES_TOOLS_REQUIRED?>/edit_collection_popup.php?<?php if ($cp->canApproveCollection() && (!$c->isEditMode())) { ?>approveImmediately=1<?php } ?>&cID=<?=$c->getCollectionID()?>&ctask=edit_metadata"><?=t('Properties')?></a></li>';
<?php } ?>
...
<?php if ($cp->canReadVersions()) { ?>
    menuHTML += '<li><a class="ccm-menu-icon ccm-icon-versions" id="ccm-toolbar-nav-versions" dialog-width="640" dialog-height="340" dialog-modal="false" dialog-title="<?=t('Page Versions')?>" href="<?=REL_DIR_FILES_TOOLS_REQUIRED?>/versions.php?cID=<?=$cID?>"><?=t('Versions')?></a></li>';
<?php } ?>
<?php if ($cp->canAdminPage()) { ?>
...
menuHTML += '<li class="ccm-main-nav-arrange-option" <?php if (!$c->isArrangeMode()) { ?> style="display: none" <?php } ?>><a href="#" id="ccm-nav-save-arrange"><?=t('Save Positioning')?></a></li>';
