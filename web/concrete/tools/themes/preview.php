...
if (isset($_REQUEST['themeID'])) {
	...
	$url = REL_DIR_FILES_TOOLS_REQUIRED . '/themes/preview_external?random=' . time() . '&themeCID=' . intval($_REQUEST['themeCID']) . '&previewCID=' . intval($_REQUEST['previewCID']) . '&themeHandle=' . htmlspecialchars($_REQUEST['themeHandle']) . '&ctID=' . intval($_REQUEST['ctID']);
}
?>
