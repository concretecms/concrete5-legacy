...
if (count($attribs) > 0) { ?>
	...
	if (count($sets) > 0 && ($_REQUEST['asGroupAttributes'] !== '0')) { ?>
		...
		<h3><?=t($as->getAttributeSetName())?></h3>
		...
		<? } else { ?>
			...
			<div class="ccm-attribute" id="akID_<?=$as->getAttributeSetID()?>_<?=$ak->getAttributeKeyID()?>">
				<img class="ccm-attribute-icon" src="<?=$ak->getAttributeKeyIconSRC()?>" width="16" height="16" /><a href="<?=$this->url($editURL, 'edit', $ak->getAttributeKeyID())?>"><?=t($ak->getAttributeKeyName())?></a>
			</div>
