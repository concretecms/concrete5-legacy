...
?>
<input type="radio" name="ccm-dashboard-display-mode" value="full" <?php if ($sitemapOverlayPreference == 'full') { ?> checked <?php } ?> onclick="ccm_sitemapSelectDisplayMode('<?=$instance_id?>', 'full', '<?=$select_mode?>', '<?=$cID?>')" /> <?=t('Full Sitemap')?>
&nbsp;&nbsp;&nbsp;&nbsp;
<input type="radio" name="ccm-dashboard-display-mode" value="explore" <?php if ($sitemapOverlayPreference == 'explore') { ?> checked <?php } ?> onclick="ccm_sitemapSelectDisplayMode('<?=$instance_id?>', 'explore', '<?=$select_mode?>', '<?=$cID?>')" /> <?=t('One Level Only')?>
</form>
</div>

<div id="tree" sitemap-wrapper="1" sitemap-select-callback="<?=$callback?>" sitemap-instance-id="<?=$instance_id?>" <?php if ($display_mode == 'explore') { ?>class="ccm-sitemap-explore"<?php } ?>>
