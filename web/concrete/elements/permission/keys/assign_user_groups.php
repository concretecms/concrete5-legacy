...
<div class="clearfix">
	...
	<ul class="inputs-list" <?php if ($assignment->getGroupsAllowedPermission() != 'C') { ?>style="display: none"<?php } ?>>
		<?php foreach($groups as $g) { ?>
			<li><label><input type="checkbox" name="gIDInclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$g->getGroupID()?>" <?php if ($assignment->getGroupsAllowedPermission() == 'A' || in_array($g->getGroupID(), $assignment->getGroupsAllowedArray())) { ?> checked="checked" <?php } ?> /> <span><?=$g->getGroupName()?></span></label></li>
		<?php } ?>
	</ul>
	</div>
</div>
...
<div class="clearfix">
	...
	<ul class="inputs-list" <?php if ($assignment->getGroupsAllowedPermission() != 'C') { ?>style="display: none"<?php } ?>>
		<?php foreach($groups as $g) { ?>
			<li><label><input type="checkbox" name="gIDExclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$g->getGroupID()?>" <?php if (in_array($g->getGroupID(), $assignment->getGroupsAllowedArray())) { ?> checked="checked" <?php } ?> /> <span><?=$g->getGroupName()?></span></label></li>
		<?php } ?>
	</ul>
	</div>
</div>
