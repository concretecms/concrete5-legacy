        ...
        <div class="ccm-pane-options">
          <strong><?=t('Set')?></strong>&nbsp;
           <select id="ccmToggleInheritance" style="width: 130px" name="cInheritPermissionsFrom">
            <?php if ($c->getCollectionID() > 1) { ?><option value="PARENT" <?php if ($c->getCollectionInheritance() == "PARENT") { ?> selected<?php } ?>><?=t('By Area of Site (Hierarchy)')?></option><?php } ?>
            <?php if ($c->getMasterCollectionID() > 1) { ?><option value="TEMPLATE"  <?php if ($c->getCollectionInheritance() == "TEMPLATE") { ?> selected<?php } ?>><?=t('By Page Type Defaults (in Dashboard)')?></option><?php } ?>
            <option value="OVERRIDE" <?php if ($c->getCollectionInheritance() == "OVERRIDE") { ?> selected<?php } ?>><?=t('Manually')?></option>
          </select>
          ...
          $isManual = ($c->getCollectionInheritance() == "OVERRIDE");
            ...
            foreach ($gArray as $g) {
                ...
                if ($display) { ?>
                    <tr id="_row:gID:<?=$g->getGroupID()?>">
                        ...
                        <td class="permissions"><input type="checkbox" name="collectionRead[]" value="gID:<?=$g->getGroupID()?>"<?php if ($g->canRead()) { ?>checked<?php } ?>></td>
                        <td class="permissions"><input type="checkbox" name="collectionReadVersions[]" value="gID:<?=$g->getGroupID()?>"<?php if ($g->canReadVersions()) { ?>checked<?php } ?>></td>

                        <td class="permissions"><input type="checkbox" name="collectionWrite[]" value="gID:<?=$g->getGroupID()?>"<?php if ($g->canWrite()) { ?>checked<?php } ?>></td>
                        <td class="permissions"><input type="checkbox" name="collectionApprove[]" value="gID:<?=$g->getGroupID()?>"<?php if ($g->canApproveCollection()) { ?>checked<?php } ?>></td>
                        <td class="permissions"><input type="checkbox" name="collectionDelete[]" value="gID:<?=$g->getGroupID()?>"<?php if ($g->canDeleteCollection()) { ?>checked<?php } ?>></td>
                           <td class="permissions"><input type="checkbox" name="collectionAdmin[]" value="gID:<?=$g->getGroupID()?>"<?php if ($g->canAdminCollection()) { ?> checked<?php } ?>></td>

                           <td class="subpage"><?php foreach ($ctArray as $ct) { ?><div style="white-space: nowrap; width: auto; float: left; margin-right: 5px; min-width: 90px"><input type="checkbox" name="collectionAddSubCollection[<?=$ct->getCollectionTypeID()?>][]" value="gID:<?=$g->getGroupID()?>"<?php if ($ct->canAddSubCollection($g)) { ?> checked<?php } ?>>&nbsp;<?=$ct->getCollectionTypeName()?></div><?php } ?><div class="ccm-spacer">&nbsp;</div></td>
            ...
            foreach ($ulArray as $ui) {
               ...
               <tr id="_row:uID:<?=$ui->getUserID()?>">
                    ...
                    <td class="permissions"><input type="checkbox" name="collectionRead[]" value="uID:<?=$ui->getUserID()?>"<?php if ($ui->canRead()) { ?>checked<?php } ?>></td>
                    <td class="permissions"><input type="checkbox" name="collectionReadVersions[]" value="uID:<?=$ui->getUserID()?>"<?php if ($ui->canReadVersions()) { ?>checked<?php } ?>></td>
                    <td class="permissions"><input type="checkbox" name="collectionWrite[]" value="uID:<?=$ui->getUserID()?>"<?php if ($ui->canWrite()) { ?>checked<?php } ?>></td>
                    <td class="permissions"><input type="checkbox" name="collectionApprove[]" value="uID:<?=$ui->getUserID()?>"<?php if ($ui->canApproveCollection()) { ?>checked<?php } ?>></td>
                    <td class="permissions"><input type="checkbox" name="collectionDelete[]" value="uID:<?=$ui->getUserID()?>"<?php if ($ui->canDeleteCollection()) { ?>checked<?php } ?>></td>
                      <td class="permissions"><input type="checkbox" name="collectionAdmin[]" value="uID:<?=$ui->getUserID()?>"<?php if ($ui->canAdminCollection()) { ?> checked<?php } ?>></td>
                      <td class="subpage">
                    <?php foreach ($ctArray as $ct) { ?>
                        <div style="white-space: nowrap; width: auto; float: left; margin-right: 5px; min-width: 90px"><input type="checkbox" name="collectionAddSubCollection[<?=$ct->getCollectionTypeID()?>][]" value="uID:<?=$ui->getUserID()?>"<?php if ($ct->canAddSubCollection($ui)) { ?> checked<?php } ?>>&nbsp;<?=$ct->getCollectionTypeName()?></div><?php } ?>
            ...
            <?php if (!$c->isMasterCollection()) { ?>
                ...
                <select id="templatePermissionsSelect" name="cOverrideTemplatePermissions">
                    <option value="0"<?php if (!$c->overrideTemplatePermissions()) { ?>selected<?php } ?>><?=t('Inherit page type default permissions.')?></option>
                    <option value="1"<?php if ($c->overrideTemplatePermissions()) { ?>selected<?php } ?>><?=t('Inherit the permissions of this page.')?></option>
                </select>
                <br><br>
                <?php } ?>
