...
<br/>
            ...
            foreach ($gArray as $g) {
                ...
                if ($display) { ?>
                    <tr class="no-bg" id="_row:gID:<?=$g->getGroupID()?>">
                        ...
                        <td><div style="text-align: center"><input type="checkbox" name="blockRead[]" value="gID:<?=$g->getGroupID()?>"<?php if ($g->canRead()) { ?> checked<?php } ?>></div></td>
                        <td><div style="text-align: center"><input type="checkbox" name="blockWrite[]" value="gID:<?=$g->getGroupID()?>"<?php if ($g->canWrite()) { ?> checked<?php } ?>></div></td>
                        <td><div style="text-align: center"><input type="checkbox" name="blockDelete[]" value="gID:<?=$g->getGroupID()?>"<?php if ($g->canDeleteBlock()) { ?> checked<?php } ?>></div></td>
                    </tr>
                <?php
                    $rowNum++;
                    } ?>
            ...
            foreach ($ulArray as $ui) { ?>
               <tr class="no-bg" id="_row:uID:<?=$ui->getUserID()?>">
                    ...
                    <td><div style="text-align: center"><input type="checkbox" name="blockRead[]" value="uID:<?=$ui->getUserID()?>"<?php if ($ui->canRead()) { ?> checked<?php } ?>></div></td>
                    <td><div style="text-align: center"><input type="checkbox" name="blockWrite[]" value="uID:<?=$ui->getUserID()?>"<?php if ($ui->canWrite()) { ?> checked<?php } ?>></div></td>
                    <td><div style="text-align: center"><input type="checkbox" name="blockDelete[]" value="uID:<?=$ui->getUserID()?>"<?php if ($ui->canDeleteBlock()) { ?> checked<?php } ?>></div></td>
                </tr>
            <?php
                $rowNum++;
                } ?>
        ...
        <a href="javascript:void(0)" onclick="<?php if ($numChildren) { ?>applyToAll();<?php } else { ?>$('#ccmBlockPermissionForm').submit()<?php } ?>" class="ccm-button-right accept btn primary"><?=t('Update')?></a>
