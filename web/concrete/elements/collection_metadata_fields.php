...
<div id="ccm-attributes-column" class="span3">
	...
	<? foreach($sets as $as) { ?>
		<li class="item-select-list-header ccm-attribute-available"><span><?=t($as->getAttributeSetName())?></span></li>
		<? 
...
<? 
	...
	foreach($attribs as $ak) {
		...
		<input type="hidden" class="ccm-meta-field-selected" id="ccm-meta-field-selected<?=$ak->getAttributeKeyID()?>" name="selectedAKIDs[]" value="<? if (!in_array($ak->getAttributeKeyID(), $usedKeysCombined)) { ?>0<? } else { ?><?=$ak->getAttributeKeyID()?><? } ?>" />
			...
			<a href="javascript:void(0)" class="ccm-meta-close" id="ccm-remove-field-ak<?=$ak->getAttributeKeyID()?>" style="display:<?=(!in_array($ak->getAttributeKeyID(), $requiredKeys) && !$ak->isAttributeKeyInternal())?'block':'none'?>"><img src="<?=ASSETS_URL_IMAGES?>/icons/remove_minus.png" width="16" height="16" alt="<?=t('remove')?>" /></a>
