...
class NavigationHelper {
	 ...
	 * 
	 * @param object Page $cObj - Starting descendant Page object to check from. Omit to use the current Page object.
	 * @param integer $nth - The nth ancestor to retrieve starting from the descendant page. Omit to retrieve the highest ancestor
	 * @return object
	 */
	public function getCollectionAncestor($cObj = NULL, $nth = NULL){		
		// Use the current Page object if nothing was passed or it's an invalid Page object
		if(!is_object($cObj) || !$cObj instanceof Page || $cObj->error) $cObj = Page::getCurrentPage();

		// We return null if we're on the home page, a system page, a master collection or the current highest ancestor
		if($cObj->cID == HOME_CID || $cObj->isSystemPage() || $cObj->isMasterCollection() || $cObj->cParentID == HOME_CID){
			return NULL;
		...
		// We return the parent Page object if we only want the immediate ancestor
			...
			return Page::getByID($cObj->cParentID);
		}
