...
class ConcreteDashboardSitemapHelper
{
    ...
    */
    ...
    public function addOpenNode($cID)
    {
    ...
    public function addOneTimeActiveNode($cID)
    {
        $_SESSION['dsbSitemapActiveNode'] = $cID;
    }

    public function clearOneTimeActiveNodes()
    {
        unset($_SESSION['dsbSitemapActiveNode']);
    }

    public function showSystemPages()
    {
        return $_SESSION['dsbSitemapShowSystem'] == 1;
    }

    public function isOneTimeActiveNode($cID)
    {
        return ($_SESSION['dsbSitemapActiveNode'] == $cID);
    }

    public function getNode($cItem, $level = 0, $autoOpenNodes = true)
    {
        if (!is_object($cItem)) {
            $cID = $cItem;
    ...
    public function getSubNodes($cID, $level = 0, $keywords = '', $autoOpenNodes = true)
    {
        $db = Loader::db();
    ...
    public function getDroppables($cID)
    {
        $db = Loader::db();
        $v = array($cID);
