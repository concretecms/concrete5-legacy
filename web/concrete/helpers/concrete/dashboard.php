		...
		ob_start(); ?>
			...
			foreach($children as $ch) {
				...
				foreach($ch2 as $chi) {
					...
					?>
					<li><a href="<?=Loader::helper('navigation')->getLinkTocollection($subpage, false, true)?>"><?=t($subpage->getCollectionName())?></a><span><? if ($page->getCollectionPath() != '/dashboard/system') { ?><?=t($page->getCollectionName())?> <?=$page->getAttribute('meta_keywords')?> <? } ?><?=t($subpage->getCollectionName())?> <?=$subpage->getAttribute('meta_keywords')?></span></li>
					<? 
				}
				?>
				</ul>
