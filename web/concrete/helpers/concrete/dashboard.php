		...
		ob_start(); ?>
			...
			foreach($children as $ch) {
				...
				foreach($ch2 as $chi) {
					...
					?>
					<li><a href="<?=Loader::helper('navigation')->getLinkTocollection($subpage, false, true)?>"><?=$subpage->getCollectionName()?></a><span><? if ($page->getCollectionPath() != '/dashboard/system') { ?><?=t($page->getCollectionName())?> <?=t($page->getAttribute('meta_keywords'))?> <? } ?><?=$subpage->getCollectionName()?> <?=t($subpage->getAttribute('meta_keywords'))?></span></li>
					<? 
				}
				?>
				</ul>
